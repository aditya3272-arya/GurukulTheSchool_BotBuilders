    ...
    async def generate_answer(
        ...
        *,
        ...
        if not retrieved_facts:
            print(" No facts received in Gemini")
        ...
        def _call() -> str:
            resp = self._client.models.generate_content(
                model=settings.gemini_model_answer,
                contents=prompt,
                config={"response_mime_type": "application/json"}
            )
            return (resp.text or "").strip()
        ...
        try:
            ...
            print("\n GEMINI ANSWER RAW:", text)
            print("\n STRUCTURED FACTS SENT:\n", formatted_facts)

            data = extract_json_object(text)

            if data:
                if not _is_valid_follow_ups(data.get("follow_ups")):
                    print(" follow_ups invalid or placeholder — regenerating via AI")
                    data["follow_ups"] = await self._generate_follow_ups(user_message, role)
                return data
